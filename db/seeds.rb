...
spotify_uris.each do |uri|
  RSpotify::Album.find(uri).artists.each do |a|
    artist = Artist.create(name: a.name)
    album = Album.find_by_name(RSpotify::Album.find(uri).name)
    AlbumAssociation.create(album: album, artist: artist)
  end
end
